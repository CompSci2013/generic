---
description: Technical code review for quality and bugs that runs pre-commit
---

Perform technical code review on recently changed files.

## Core Principles

Review Philosophy:

- Simplicity is the ultimate sophistication - every line should justify its existence
- Code is read far more often than it's written - optimize for readability
- The best code is often the code you don't write
- Elegance emerges from clarity of intent and economy of expression

## What to Review

Start by gathering codebase context to understand the codebase standards and patterns.

Start by examining:

- CLAUDE.md
- docs/claude/ORIENTATION.md
- .claude/PRD.md

After you have a good understanding

Run these commands:

```bash
git status
git diff HEAD
git diff --stat HEAD
```

Then check the list of new files:

```bash
git ls-files --others --exclude-standard
```

Read each new file in its entirety. Read each changed file in its entirety (not just the diff) to understand full context.

For each changed file or new file, analyze for:

1. **Logic Errors**
   - Off-by-one errors
   - Incorrect conditionals
   - Missing error handling
   - Race conditions in async code

2. **Security Issues**
   - Command injection in run_command tool
   - Path traversal in file operations
   - Exposed secrets or API keys

3. **Performance Problems**
   - Blocking calls in async functions
   - Memory leaks in streaming
   - Unnecessary model reloading

4. **Code Quality**
   - Violations of DRY principle
   - Overly complex functions
   - Poor naming
   - Missing type hints (Python)

5. **MLX Code Specific**
   - SSE event format consistency
   - Tool registration patterns
   - Thor ↔ Mimir communication
   - Error propagation through SSE

6. **Adherence to Patterns**
   - Existing code style in server.py
   - Existing JavaScript patterns in chat.js
   - SSE event types and format

## Verify Issues Are Real

- Run syntax checks for issues found
- Confirm security concerns with context
- Test error handling paths

## Output Format

Save a new file to `.agents/code-reviews/[appropriate-name].md`

**Stats:**

- Files Modified: 0
- Files Added: 0
- Files Deleted: 0
- New lines: 0
- Deleted lines: 0

**For each issue found:**

```
severity: critical|high|medium|low
file: path/to/file.py
line: 42
issue: [one-line description]
detail: [explanation of why this is a problem]
suggestion: [how to fix it]
```

If no issues found: "Code review passed. No technical issues detected."

## Important

- Be specific (line numbers, not vague complaints)
- Focus on real bugs, not style
- Suggest fixes, don't just complain
- Flag security issues as CRITICAL
- Pay special attention to async/await patterns
- Check SSE event consistency
